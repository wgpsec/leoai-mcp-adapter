...
2. If the task is to bring a Java execution point under LeoAI management, follow `leoai-onboard-memory-shell` instead of improvising delivery. Creating a Project is not a Puppet. Generating an artifact is not completed onboarding. The Adapter does not deliver artifacts. HTTP/httpchunk memory shells require `headerName` and `headerValue`; JDK 9+ needs `targetJavaVersion`; Spring Boot 3 / Tomcat 10 needs `servletNamespace=jakarta`. Injector instantiation is not a hooked shell: verify the header gate before `leo_add_puppet`.
